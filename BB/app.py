...
# Database Connection
# Credentials now come from environment variables instead of being hardcoded.
...
def signup():
    if request.method == "POST":
        ...
        cursor.execute("insert into login values(%s,%s)", (username, generate_password_hash(password)))
        conn.commit()
...
def add_transaction():
    ...
    if request.method == "POST":
        ...
        global user
        sql = "INSERT INTO transactions (user,date, type, category, amount, note) VALUES (%s,%s,%s,%s,%s,%s)"
        cursor.execute(sql, (user, date, t_type, category, amount, note))
        conn.commit()
...
def view_all():
    global user
    cursor.execute("SELECT * FROM transactions where user=%s ORDER BY date DESC", (user,))
...
@app.route("/summary")
def summary():
    cursor.execute("SELECT SUM(amount) FROM transactions WHERE type='Income' and user=%s", (user,))
    income = cursor.fetchone()[0] or 0

    cursor.execute("SELECT SUM(amount) FROM transactions WHERE type='Expense' and user=%s", (user,))
...
def clearall():
    ...
    r=cursor.fetchall()
    for i in r:
        cursor.execute("delete from transactions where id=%s and user=%s", (i[0], user))
        conn.commit()
    return redirect("/view")
